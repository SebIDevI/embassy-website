...
/* Parse a display stat ("21K", "+340%", "4.6%", "5K€", "-38%") into an
   animatable number + its fixed prefix/suffix. Anything that doesn't start
...
function parse(value: string) {
  ...
  return { sign, target, decimals, suffix };
}
...
export default function CountUp({
  ...
  useEffect(() => {
    ...
    if (!parsed || !el) return;
    if (reduce) {
      el.textContent = fmt(parsed.target, parsed.decimals);
      return;
    }
    if (!inView) {
      el.textContent = fmt(0, parsed.decimals); // reset so it re-counts on re-entry
      return;
    }
    el.textContent = fmt(0, parsed.decimals); // start from 0, no target flash
    const obj = { v: 0 };
    const tween = gsap.to(obj, {
      ...
      ease: "power3.out",
      onUpdate: () => {
        el.textContent = fmt(obj.v, parsed.decimals);
      },
    });
    return () => {
      tween.kill();
  ...
  return (
    ...
    >
      {parsed ? (
        <>
          {parsed.sign}
          <span ref={numRef}>{fmt(parsed.target, parsed.decimals)}</span>
          {parsed.suffix}
        </>
      ) : (
        value
